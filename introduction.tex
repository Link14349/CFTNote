...
\begin{align}
    g\to\phi_g, \phi_gf|_{p}\equiv \mathcal F\phi_{*t=1}f|_p.
\end{align}
其中, $\phi$对应的光滑矢量场即$\omega_a K_a$, $\mathcal F$代表场的变换缩放.

于是有
\begin{align}
    (1-i\epsilon a^\mu p_\mu-\frac i2\epsilon\omega^{\mu\nu}M_{\mu\nu}-i\epsilon\lambda D-i\epsilon b^\mu K_\mu)f(x^\mu)=\mathcal Ff(x-\epsilon\xi^\mu).
\end{align}
所以可以计算得
\begin{align}
    & p_\mu=-i\partial_\mu, \\
    ...
    & K_\mu=-i(2x_\mu x^\nu\partial_\nu-x^2\partial_\mu-2\Delta x_\mu)+2x^\nu S_{\mu\nu}.
\end{align}
注意, 其中$S_{\mu\nu}$为自旋矩阵, 对于标量场来说其为$0$, 但是为了结论具有普适性, 我们希望其对于有自旋的粒子也成立, 因此我们添加了这项.
...
然后我们考虑$D, K_\mu$, 它们的变换会比较麻烦一些, 因为同时包含移动与缩放. 比较简单的方式是利用Dilation与SCT不改变原点位置的特性, 首先计算$\phi(0)$与$D, K_\mu$的对易子.

对于Dilation, $\sigma=\frac1d\partial_\mu\xi^\mu=\lambda$, $\phi(0)$的缩放为$\phi(0)\to\Omega^{-\Delta}\phi=\exp{\Delta\sigma}\phi$, 因此
\begin{align}
    \exp{i\lambda D}\phi(0)\exp{-i\lambda D}=\exp{\Delta\lambda}\phi(0),
\end{align}
于是有对易子
\begin{align}
...
对于SCT, $\sigma(0)=\frac1d\partial_\mu\xi^\mu=0$, 因此
\begin{align}
    \exp{iK_\mu B^\mu}\phi(0)\exp{-iK_\mu B^\mu}=\phi(0),
\end{align}
于是
\begin{align}
    [K_\mu, \phi(0)]=0.
\end{align}
这样的$\phi(0)$称为主算符(Primary Operator).

对于$D, K_\mu$与$\phi(x)$处的对易子可以使用$\phi(x)=\exp{-ipx}\phi(0)\exp{ipx}$间接计算:
\begin{align}
    [D, \exp{ipx}\phi(0)\exp{-ipx}]&=\exp{ipx}[\exp{-ipx}D\exp{ipx}, \phi(0)]\exp{-ipx}.
\end{align}
而
\begin{align}
    \exp{-ipx}D\exp{ipx}&=D+[-ipx, D]+\frac1{2!}[-ipx, [-ipx, D]]+\cdots\\
    &=D-px
\end{align}
于是
\begin{align}
...
类似地,
\begin{align}
    \exp{-ipx}K_\mu\exp{ipx}&=K_\mu+2(M_{\mu\nu}x^\nu-x_\mu D)+(2x_\mu x^\nu p_\nu-x^2 p_\mu),
\end{align}
从而有
\begin{align}
